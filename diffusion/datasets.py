...
from diffusion.dataset.coco import CocoSegmentation
...
def get_target_dataset(name: str, train=False, transform=None, target_transform=None):
    ...
    if name == "cifar10":
        ...
        # assert not train # not sure why this is here
        dataset = datasets.ImageFolder(root=osp.join(DATASET_ROOT, 'imagenet/train' if train else 'imagenet/val'),
                                       transform=transform,
        ...
        dataset = CocoSegmentation(images_root=COCO_DATASET_ROOT, split='train2017' if train else "val2017")
        dataset.class_to_idx = None
        dataset.file_to_class = None
    elif name == "nyudepthv2":
